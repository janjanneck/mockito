    ...
    public static MockitoException misplacedArgumentMatcher(List<LocalizedMatcher> lastMatchers) {
        return new InvalidUseOfMatchersException(
                join(
                        ...
                        "",
                        "There are several reasons as to why this message might appear:",
                        "1. You used argument matchers outside of verification or stubbing.",
                        "   Examples of correct usage of argument matchers:",
                        "       when(mock.get(anyInt())).thenReturn(null);",
                        "       doThrow(new RuntimeException()).when(mock).someVoidMethod(any());",
                        "       verify(mock).someMethod(contains(\"foo\"))",
                        "",
                        "2. The stubbed method signature expects a primitive argument,",
                        "   but the last checked matcher returned an object (like any()).",
                        "   Look for a NullPointerException immediately before this message.",
                        "   In this case, use primitive alternatives:",
                        "       when(mock.get(anyInt())); // correct usage",
                        "       when(mock.get(any())); // bad usage, will raise NPE",
                        "",
                        "3. You used argument matchers with methods that cannot be mocked.",
                        "   The following methods *cannot* be stubbed/verified: final/private/equals()/hashCode().",
                        "4. " + NON_PUBLIC_PARENT,
                        ""));
    }
